 ...
 *
 * Adopted from cronologia/core template/scripts/translate.js. The only local
 * adaptation is the dataset: this repo's source of truth is data/glossary.json
 * (terms[] + references[]), not data/chronology.json, so the key set adds the
 * glossary's own prose fields (`term`, `definition`, `variants`). Term ids are
 * permanent URLs and are never collected, so /glossary/<id>/ is identical in
 * every locale.
 */
'use strict';
...
const TRANSLATABLE_KEYS = new Set([
  ...
  // glossary-specific prose (ids are URLs and stay untranslated)
