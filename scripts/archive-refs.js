 ...
 *
 * Zero dependencies. Reads the `references[]` array of data/glossary.json,
 ...
 * (plus a fresh re-capture for references whose `type` is "primary" — the
 * official/primary-source documents — once their latest snapshot is older
 * than the refresh window). Results are written to data/archives.json, which
 * build.js reads to render "archived" fallback links next to each reference.
 *
 ...
 *     with a snapshot are skipped on re-runs (primary refs are re-captured
 ...
 *        ARCHIVE_REFRESH_DAYS   re-capture window for primary refs (default 7)
 *
...
const USER_AGENT =
  'cronologia-archive-refs/1.0 (Cronologia glossary project; +https://github.com/cronologia/glossary)';
...
/** A reference is treated as official/primary when its `type` is "primary". */
function isOfficialRef(ref) {
  return ref.official === true || ref.type === 'primary';
}
...
async function main() {
  ...
  for (const ref of references) {
    ...
    const isOfficial = isOfficialRef(ref);
    const label = `${ref.id || '?'} ${url}`;

    // Idempotence: skip what archives.json already has, unless this is a
    // primary reference whose snapshot has aged past the refresh window.
    if (existing && existing.archiveUrl) {
      ...
      console.log(`re-capturing primary ref: ${label}`);
