...
// This repo declares its own fallback name and contact URL inside the
// `>>> ADOPT: user-agent` block of scripts/check-links.js, so the expected
// strings here differ from the template's. Only the ADOPT-block values differ;
// the headerSafe guard below is shared machinery and is asserted verbatim.
test('deriveUserAgent names the project and has a contact URL', () => {
  assert.match(deriveUserAgent('RCC Brasil'), /^cronologia-check-links\/1\.0 \(RCC Brasil; \+https:\/\/github\.com\/cronologia\/glossary\)$/);
  assert.match(deriveUserAgent(''), /Cronologia glossary/);
  assert.match(deriveUserAgent(undefined), /Cronologia glossary/);
});
